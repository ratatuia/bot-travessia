...
class TelegramService:
    ...
    def adicionar_mensagem(self, sender, nome_cliente, cliente_msg, bot_msg, tipo="mensagem"):
        ...
        if sender not in self.conversas:
            self.conversas[sender] = {
                ...
                "perfil": {},
                "historico": []  # NOVO: histórico de conversas anteriores
            }
    ...
    def enviar_conversa(self, sender, forcar=False, lead_score=0):
        """Envia a conversa acumulada para o Telegram com lead score"""
        if sender not in self.conversas:
            return False
        ...
        # NOVO: Emoji baseado em lead score
        if lead_score >= 70:
            emoji_prioridade = "🔥🔥🔥"  # QUENTE!
        elif lead_score >= 50:
            emoji_prioridade = "⭐⭐"  # Bom lead
        else:
            emoji_prioridade = "📋"  # Lead normal

        # Formata o cabeçalho
        cabecalho = (
            f"{emoji_prioridade} *{nome.upper()}*\n"
            f"📱 [{link_whatsapp}](https://{link_whatsapp})\n"
            f"🕒 {datetime.datetime.now(BRASIL_TZ).strftime('%d/%m/%Y %H:%M')}\n"
        )

        # NOVO: Mostra lead score se > 0
        if lead_score > 0:
            cabecalho += f"🎯 *Lead Score*: {lead_score}/100\n"
...
🔧 Ação necessária!"""
            ...
            botoes = [
                {"text": "🏥 Ver Health", "url": "https://bot-travessia.onrender.com/dashboard"},
                {"text": "📊 Ver Dashboard", "url": "https://bot-travessia.onrender.com/dashboard"}
            ]
